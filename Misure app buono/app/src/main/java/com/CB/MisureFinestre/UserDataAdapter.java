...
public class UserDataAdapter extends RecyclerView.Adapter<UserDataAdapter.ViewHolder> {
    ...
    // 🔹 IMPORTANT METHOD (YOUR ERROR FIX)
    public void updateList(List<CustomerCacheEntity> newList) {
        list.clear();
        list.addAll(newList);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        CustomerCacheEntity item = list.get(position);
        ...
        holder.btnDupli.setOnClickListener(v -> {
            Intent i = new Intent(context, AddFormOneActivity.class);
            i.putExtra("CUSTOMER_ID", item.id);
            i.putExtra("DOUBLICAT", "doublicat");
            context.startActivity(i);
        });
        ...
        holder.btnPdf.setOnClickListener(v -> {
            ...
            try {
                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(item.pdf_url));
...
//            if (item.isOffline) {
//                Toast.makeText(context, "This customer is offline & not synced yet", Toast.LENGTH_SHORT).show();
//                return;
//            }
